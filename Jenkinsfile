pipeline {
    ...
    stages {
        ...
        stage('Run test & generate coverage and SBOM & update IAP annotation'){
            steps {
                script {
                    parallel(
                        ...
                        "Check & Update IAP application information": {
                            if (env.IS_DEPLOY_ON_S3 == 'true'){
                                if (checkAnnotationNotExist(IAP_ANNOTATION, FILE_PATH)) {
                                    echo "🔍 iap/app-info is empty. Fetching application information..."
                                    ...
                                    if (applicationId && applicationId != "null") {
                                        ...
                                        updateCatalogInfo(IAP_ANNOTATION, "${IAP_PROJECT_ID}/${applicationId}")

                                        commitAndPushChanges("Updated iap/app-info in catalog-info.yaml")

                                        deregisterWithBackstage()

                                        registerWithBackstage()
                                    } else {
                                        error "❌ Failed to fetch IAP application information!"
                                    }
                                } else {
                                    echo "✅ IAP application information is exist. Skipping update."
                                }
                            } else {
                                echo "🚀 Skipping IAP application information update since IS_DEPLOY_ON_S3 is not true."
                            }
                        }
                    )
                }
...
def fetchApplicationUUID() {
    def response = sh(script: """
        curl -s -X 'GET' '${IAP_BACKEND_URL}/${IAP_PROJECT_ID}/applications' \
